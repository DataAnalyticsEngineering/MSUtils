...
class MicrostructureImage:
    def __init__(self, h5_filename=None, dset_name=None, image=None, resolution=None, L=[1,1,1], metadata=None):
    ...
    def write(self, h5_filename=None, dset_name=None, order="xyz"):
        ...
        if order == "xyz":
            ...
            permuted_image = np.transpose(self.image, (2, 1, 0))  # Permute dimensions from xyz to zyx
        else:
        ...
        with h5py.File(self.h5_filename, 'a') as f:
            ...
            dset = f.create_dataset(self.dset_name, data=permuted_image, compression="gzip", compression_opts=9)
            
            if self.metadata:
                for key, value in self.metadata.items():
                    dset.attrs[key] = value

            print(f"Image written to dataset {self.dset_name} in {self.h5_filename} with GZIP level 9 compression.")
