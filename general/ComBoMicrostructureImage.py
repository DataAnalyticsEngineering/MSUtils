...
class ComBoMicrostructureImage:
    ...
    def get_image_laplacian(self, img, L=[1., 1., 1.]):
        """
    ...
    def downscale(self, data_array, Nx, Ny, Nz, min_vol_fraction=0.0, L=[1., 1., 1.], pad_window=[0, 0, 0]):
        """
            ...
            for j in range(Ny):
                for k in range(Nz):
                    ...
                    if len(significant_materials) == 2:
                        ...
                        normal =  self.supervoxel_normal( supervoxel, lap_img = iface_block, l = l_vx, vol_frac = significant_fractions[1])
    ...
    def write(self, filename, group_name):
        ...
        with h5py.File(filename, 'a') as f:
            ...
            if self.coarse_image is not None:
                grp.create_dataset("coarse_image", data=self.coarse_image, dtype=int, compression="gzip", compression_opts=9)
            
            if self.fine_image is not None:
                grp.create_dataset("fine_image", data=self.fine_image, dtype=np.uint8, compression="gzip", compression_opts=9)
            ...
            if self.voxel_info_list:
                if self.coarse_image is not None:
                    coarse_normal = np.zeros((self.coarse_image.shape[0], self.coarse_image.shape[1], self.coarse_image.shape[2], 3))
                    for voxel_info in self.voxel_info_list:
                    ...
                    grp.create_dataset("coarse_normal", data=coarse_normal, dtype='f4', compression="gzip", compression_opts=9)

                max_materials = max(len(voxel_info.materials) for voxel_info in self.voxel_info_list)
                ...
                grp.create_dataset("multi_material_voxels", data=structured_array, dtype=dtype, compression="gzip", compression_opts=9)
        ...
        with h5py.File(filename, 'r') as f:
            ...
            if "multi_material_voxels" in group:
                ...
                for voxel in voxel_data:
                    ...
                    voxel_info = VoxelInfo(coords, elem_xyz, materials, fractions, fine_scale_block, normal)
